...
func (tx *Tx) init() {
	tx.ADGroup = NewADGroupClient(tx.config)
	tx.Application = NewApplicationClient(tx.config)
	tx.ApplicationCategory = NewApplicationCategoryClient(tx.config)
	...
	tx.PartnerPageLinkFragment = NewPartnerPageLinkFragmentClient(tx.config)
}
...
// applies a query, for example: ADGroup.QueryXXX(), the query will be executed
